class mydevops_blog::doprod_web($blogweb=true) {

if $blogweb {
    
      include mydevops_blog::doprod_web::blogweb
      
  } else {
  
    notify {'Cam web setup seems disabled':}
    
  }
}

class mydevops_blog::doprod_web::blogweb{
...
class { 'nginx::config' :
    ...
    http_cfg_append => {
      'passenger_max_pool_size' => hiera('blog_passenger_max_pool_size', 24),
    }
  }
...
nginx::resource::vhost { 'logview':
  ...
  listen_port          => '80',
  raw_append           => [ 'real_ip_header proxy_protocol;', 'set_real_ip_from 10.0.0.0/8;' ],
...
nginx::resource::vhost { 'accounts.mydevops.com-no-ssl':
  ...
  listen_port      => '80',
  raw_prepend      => [ 'listen       *:8080 proxy_protocol;'],
  raw_append       => [ 'real_ip_header proxy_protocol;', 'set_real_ip_from 10.0.0.0/8;' ],
...
nginx::resource::vhost { 'accounts.mydevops.com-ssl':
  ...
  server_name               => ['accounts.mydevops.com'],
  listen_port               => '443',
  raw_prepend               => [ 'listen *:4430 ssl proxy_protocol;'],
  raw_append                => [ 'real_ip_header proxy_protocol;', 'set_real_ip_from 10.0.0.0/8;' ],
  vhost_cfg_ssl_prepend     => {
  'passenger_ruby'          => '/usr/local/rvm/wrappers/ruby-2.6.2@blog5/ruby',
  'passenger_enabled'       => 'on',
  'passenger_app_env'       => 'production',
  'passenger_start_timeout' => hiera('blog_passenger_start_timeout', '300'),
  'passenger_max_requests'  => hiera('blog_passenger_max_requests', 30000),
  'passenger_min_instances' => hiera('blog_passenger_min_instances', 16),
  },
  use_default_location      => false,
  ssl                       => true,
  ...
  vhost_cfg_ssl_append      => { 
    ...
    'add_header' => [ 'Strict-Transport-Security "max-age=31536000; includeSubdomains; preload"', 'Content-Security-Policy "default-src \'self\'; script-src \'self\' \'unsafe-inline\' \'unsafe-eval\' data: https://ads.bridgetrack.com https://ssl.google-analytics.com https://www.google-analytics.com https://ajax.googleapis.com https://cdnjs.cloudflare.com https://script.hotjar.com https://static.hotjar.com https://plugins.help.com https://maxcdn.bootstrapcdn.com https://secure.mydevops.net https://www.mydevopslogo.com https://secure.mydevops.com https://cwatch.mydevops.com https://cdn.optimizely.com https://www.google.com https://www.gstatic.com ; style-src \'self\' \'unsafe-inline\' https://maxcdn.bootstrapcdn.com https://cdnjs.cloudflare.com https://fonts.googleapis.com ; font-src \'self\' https://maxcdn.bootstrapcdn.com https://fonts.gstatic.com ; img-src \'self\' data: https://stats.g.doubleclick.net https://secure.mydevops.com https://cwatch.mydevops.com https://www.google-analytics.com https://www.mydevopslogo.com https://ssl.google-analytics.com ; child-src https://blog.mydevops.com https://plugins.help.com https://vars.hotjar.com https://www.mydevopslogo.com https://secure.mydevops.net https://secure.mydevops.com https://cwatch.mydevops.com https://www.youtube.com https://www.google.com ; object-src \'self\' https://secure.mydevops.net https://secure.mydevops.com https://cwatch.mydevops.com https://www.youtube.com ; connect-src \'self\' https://accounts.mydevops.com https://insights.hotjar.com ; report-uri https://cspreports.mydevops.com; frame-ancestors https://blog.mydevops.com"',  'X-Content-Type-Options "nosniff" always', 'X-Frame-Options SAMEORIGIN always' ],
  },
...
nginx::resource::vhost { 'accounts.mydevopslive.com-no-ssl':
  ensure           => present,
  server_name      => ['accounts.mydevopslive.com'],
  listen_port      => '80',
  raw_prepend      => [ 'listen *:8080 proxy_protocol;'],
  raw_append       => [ 'real_ip_header proxy_protocol;', 'set_real_ip_from 10.0.0.0/8;' ],
  www_root         => '/home/sites/accounts.mydevops.com/public',
...
nginx::resource::vhost { 'accounts.mydevopslive.com-ssl':
  ensure                => present,
  server_name           => ['accounts.mydevopslive.com'],
  listen_port           => '443',
  raw_prepend           => [ 'listen *:4430 ssl proxy_protocol;'],
  ...
  use_default_location  => false,
  ssl                   => true,
  ssl_cert              => '/etc/nginx/ssl/accounts.mydevopslive.com/accounts_mydevopslive_com.crt',
  ssl_key               => '/etc/nginx/ssl/accounts.mydevopslive.com/accounts_mydevopslive_com.key',
  www_root              => '/home/sites/accounts.mydevops.com/public',
  vhost_cfg_ssl_append  => { 
    ''  => 'if (-f /var/www/html/outage.txt) {
 ...
 file {
  ...
  file {
   'ssl crt and key files of accounts.mydevopslive.com':
    ensure => 'directory',
    source => 'puppet:///modules/mydevops_blog/ssl/accounts.mydevopslive.com',
    recurse => true,
    path => '/etc/nginx/ssl/accounts.mydevopslive.com',
   }
 ...
 ## blog log rotate file
  file{'//etc/logrotate.d/blog-logrotate':
    ensure  => present,
    owner   => root,
    group   => root,
    mode    => '0644',
    source  => 'puppet:///modules/mydevops_blog/blog-logrotate',
  }
 ## blog log backup to cloud
  file{'/root/log-sync.sh':
    ensure  => present,
    owner   => root,
    group   => root,
    mode    => '0700',
    content => template('mydevops_blog/log-sync.sh.erb'),
  }
}
